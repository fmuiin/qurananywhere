<template>
    <div class="surah-detail flex flex-col min-h-screen transition-colors duration-300" :class="darkMode ? 'bg-dark-bg text-dark-text' : 'bg-light-bg text-light-text'">
      <!-- Navbar -->
      ...
      <main class="flex-grow container mx-auto p-6">
        <router-link to="/quran" class="hover:text-primary-blue transition duration-200 mb-6 inline-block text-lg font-medium" :class="darkMode ? 'text-secondary-green' : 'text-secondary-green'">
          ← Back to Surahs
        </router-link>
        <div v-if="surah" class="animate-fade-in">
          <div class="flex justify-between items-center mb-6">
            ...
            <button
              ...
              class="text-2xl hover:scale-110 transition duration-200"
              :class="isFavorite(surah.number) ? 'text-accent-gold' : 'text-gray-400'"
            >
              <i :class="isFavorite(surah.number) ? 'fas fa-star' : 'far fa-star'"></i>
            </button>
          </div>
          <p class="mb-6 text-lg" :class="darkMode ? 'text-dark-text' : 'text-light-text'">Verses: {{ surah.numberOfAyahs }}</p>
          <div class="mb-8">
            <button
              @click="playSurah"
          ...
          <div v-if="loading" class="text-lg" :class="darkMode ? 'text-dark-text' : 'text-light-text'">Loading verses...</div>
          <div v-else-if="error" class="text-lg text-red-600">{{ error }}</div>
          <div v-else class="space-y-6">
            <div
              v-for="(ayah, index) in surah.ayahs"
              :key="ayah.number"
              @click="playSingleAyah(index)"
              ...
              :class="darkMode ? 'bg-dark-nav' : 'bg-white'"
            >
              <p class="text-xl font-arabic" :class="darkMode ? 'text-white' : 'text-primary-blue'">{{ ayah.numberInSurah }}. {{ ayah.text }}</p>
              ...
              <button
                @click.stop="toggleVerseBookmark(surah.number, ayah.numberInSurah)"
